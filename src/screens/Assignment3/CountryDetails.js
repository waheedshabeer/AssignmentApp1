...
const CountryDetails = ({navigation, route}) => {
  ...
  const handleWeather = async () => {
    const res = await axios.get(
      `http://api.weatherstack.com/current?access_key=5a0b3006ad3c13b2cb36d86bac22b78c&query=${item[0]?.capital}`,
    );
    if (res?.data) {
      setData(res?.data);
      setShowWeather(true);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.value}>
          {item[0]?.latlng[0] + ' ' + item[0]?.latlng[1]}
        </Text>
      </View>
      {showWeather ? (
        <>
