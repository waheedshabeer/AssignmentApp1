...
const Home = ({navigation}) => {
  ...
  const handleSubmit = async () => {
    const res = await axios.get(
      `https://api.nasa.gov/neo/rest/v1/neo/${userId}?api_key=4smUeBEG8zNggkHNEFELPXuNM5J5PChjpjraaVgJ`,
    );
    if (res?.data) {
  ...
  const handleRandom = async () => {
    ...
    const random = await axios.get(
      `https://api.nasa.gov/neo/rest/v1/neo/${val}?api_key=4smUeBEG8zNggkHNEFELPXuNM5J5PChjpjraaVgJ`,
    );
